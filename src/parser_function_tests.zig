...
test "parser multiline function with multiline generic return type" {
    ...
    }
    {
        ...
        try parser.nodes.expectEqualsSlice(&[_]Node{
            // [0]:
            Node{ .enclosed = .{ .open = .none, .tab = 0, .start = 1 } }, // \t...\b at tab 0
            Node{ .statement = .{ .node = 40, .next = 0 } }, // statement ... 	 {...}
            Node{ .callable_token = 1 }, // gezerat
            Node{ .enclosed = .{ .open = .paren, .tab = 0, .start = 4 } }, // (...) at tab 0
            Node{ .statement = .{ .node = 5, .next = 0 } }, // statement \t...\b at tab 4
            // [5]:
            ...
            Node{ .statement = .{ .node = 19, .next = 0 } }, // statement [...] at tab 4
            Node{ .enclosed = .{ .open = .bracket, .tab = 4, .start = 20 } }, // [...] at tab 4
            // [20]:
            Node{ .statement = .{ .node = 21, .next = 0 } }, // statement \t...\b at tab 8
            Node{ .enclosed = .{ .open = .none, .tab = 8, .start = 22 } }, // \t...\b at tab 8
            Node{ .statement = .{ .node = 25, .next = 26 } }, // statement type0 : int
            Node{ .callable_token = 25 }, // type0
            Node{ .callable_token = 29 }, // int
            // [25]:
            Node{ .binary = .{ .operator = .op_declare_readonly, .left = 23, .right = 24 } }, // type0 : int
            Node{ .statement = .{ .node = 29, .next = 0 } }, // statement type1 : str
            Node{ .callable_token = 31 }, // type1
            Node{ .callable_token = 35 }, // str
            Node{ .binary = .{ .operator = .op_declare_readonly, .left = 27, .right = 28 } }, // type1 : str
            // [30]:
            Node{ .binary = .{ .operator = .op_indent, .left = 16, .right = 17 } }, // ... 	 \t...\b
            Node{ .enclosed = .{ .open = .brace, .tab = 0, .start = 32 } }, // {...} at tab 0
            Node{ .statement = .{ .node = 33, .next = 0 } }, // statement \t...\b at tab 4
            Node{ .enclosed = .{ .open = .none, .tab = 4, .start = 34 } }, // \t...\b at tab 4
            Node{ .statement = .{ .node = 39, .next = 0 } }, // statement go_bular   (...)
            // [35]:
            Node{ .callable_token = 41 }, // go_bular
            Node{ .enclosed = .{ .open = .paren, .tab = 4, .start = 37 } }, // (...) at tab 4
            Node{ .statement = .{ .node = 38, .next = 0 } }, // statement Argument0
            Node{ .atomic_token = 45 }, // Argument0
            Node{ .binary = .{ .operator = .op_access, .left = 35, .right = 36 } }, // go_bular   (...)
            // [40]:
            Node{ .binary = .{ .operator = .op_indent, .left = 30, .right = 31 } }, // ... 	 {...}
            .end, // end
        });
        // No tampering done with the file, i.e., no errors.
        try parser.tokenizer.file.expectEqualsSlice(&file_slice);
